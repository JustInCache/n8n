...
export = {
	...
	getFolder: [
		...
		async (req: AuthenticatedRequest<{ projectId: string; folderId: string }>, res: Response) => {
			const projectId = await resolveProjectId(req.user, req.params.projectId, ['folder:read']);
	...
	updateFolder: [
		...
		async (req: AuthenticatedRequest<{ projectId: string; folderId: string }>, res: Response) => {
			const projectId = await resolveProjectId(req.user, req.params.projectId, ['folder:update']);
