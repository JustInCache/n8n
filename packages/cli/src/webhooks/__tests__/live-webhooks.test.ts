...
describe('LiveWebhooks', () => {
	...
	describe('route family scoping', () => {
		...
		it('executes a form trigger on the form route family', async () => {
			...
			const formWebhooks = new LiveFormWebhooks(
				mockLogger(),
				nodeTypes,
				webhookService,
				workflowRepository,
			...
			await expect(
				formWebhooks.executeWebhook(buildRequest(), mock<Response>()),
		...
		it('executes an mcp trigger on the mcp route family', async () => {
			...
			const mcpWebhooks = new LiveMcpWebhooks(
				mockLogger(),
				nodeTypes,
				webhookService,
				workflowRepository,
			...
			await expect(
				mcpWebhooks.executeWebhook(buildRequest(), mock<Response>()),
			).resolves.toBeDefined();
		});
		...
		it('findAccessControlOptions returns undefined when the node type does not match the route family', async () => {
			setupMocks('form', 'n8n-nodes-base.formTrigger');

			const options = await liveWebhooks.findAccessControlOptions(webhookPath, httpMethod);
