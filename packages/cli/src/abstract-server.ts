...
import { LiveFormWebhooks, LiveMcpWebhooks, LiveWebhooks } from '@/webhooks/live-webhooks';
import { TestFormWebhooks, TestMcpWebhooks, TestWebhooks } from '@/webhooks/test-webhooks';
	...
	async start(): Promise<void> {
		...
		if (this.webhooksEnabled) {
			...
			this.app.all(
				...
				createWebhookHandlerFor(Container.get(LiveFormWebhooks)),
			);

			// Register a handler for live webhooks
			this.app.all(
				`/${this.endpointWebhook}/*path`,
				createWebhookHandlerFor(Container.get(LiveWebhooks)),
			);
			...
			this.app.all(
				...
				createWebhookHandlerFor(Container.get(LiveMcpWebhooks)),
			);
		}
		...
		if (this.testWebhooksEnabled) {
			this.app.all(
				...
				createWebhookHandlerFor(Container.get(TestFormWebhooks)),
			);
			this.app.all(
				`/${this.endpointWebhookTest}/*path`,
				createWebhookHandlerFor(Container.get(TestWebhooks)),
			);

			// Register a handler for test MCP servers
			this.app.all(
				`/${this.endpointMcpTest}/*path`,
				createWebhookHandlerFor(Container.get(TestMcpWebhooks)),
			);
		}
